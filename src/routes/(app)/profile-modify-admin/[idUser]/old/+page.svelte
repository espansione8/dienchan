<script lang="ts">
	...
	let businessCategory = $state('Certifications & Notary');
	...
	let docModifyArray = $state(userData.docModifyArray || []);
	let membershipArray = $state(userData.membership || []);
	let grossIncome = $state('');
	let role = $state('');
	let bday = $state('Day');
	let bMonth = $state('Month');
	let bYear = $state('Year');
		...
		<form class="card-body pt-1" action="">
			<fieldset disabled={closedInput} class="grid grid-cols-12 gap-x-4 gap-y-8">
				...
				<div class="form-control col-span-12 md:col-span-6">
					<label for="country" class="form-label">
						<div class="flex items-center gap-4">
							<span class="label-text font-bold mt-2"
								>Livello di permesso (solo per SuperAdmin)</span
							>
						</div>
						<select
							id="level"
	...
	<section class="card col-span-12 xl:col-span-6 gap-y-8 rounded-lg bg-white">
		...
		<div class="card-body pt-1">
			<form class="card-body" onsubmit={(e) => onFileUpload(e, 'avatar')}>
				<!-- <form class="card-body" onsubmit={logo_upload_submit}> -->
				<div class="alert bg-primary glass font-bold flex justify-between">
					<span><Camera /> Profile Picture</span>
					{#if picFilter[0]?.type == 'avatar'}
						<button
							type="button"
							onclick={() => onPicDelete(picFilter[0].filename, 'avatar')}
							class="btn btn-error btn-circle btn-sm"><Trash2 /></button
						>
					{/if}
				</div>
				{#if picFilter.length == 0}
					<div class="form-control">
						<label class="form-control">
							<div class="label">
								<span class="label-text">choose your photo</span>
							</div>
							<div class="join">
								<input
									type="file"
