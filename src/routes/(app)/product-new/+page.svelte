<script lang="ts">
	...
	import { SquareX, CloudUpload, Pen, Users, List, Calculator, FileDown } from 'lucide-svelte';
	...
	let error = $state();
	let productTitolo = $state('');
	let productDescrizione = $state('');
	let productUserId = $state(userData.userId);
	let productStatus = $state('enabled');
	let productCategoria = $state('');
	let productElencoTag = $state([]);
	let productInputTag = $state('');
	let productQuantita = $state(0);
	// let eventoProdotti;
	// let productElencoEmailNotifica = [userData.email];
	let productInputEmailNotifica = $state('');
	let productCost = $state(0);
	...
	async function submitForm(event: any) {
		...
		// data['directory'] = dir;
		data['directory'] = 'immagini';
		data['reqType'] = 'upload';
		const response = await fetch(`/api/products/register-product`, {
			method: 'POST',
	...
	const deleteFunctionJPG = async (fileName) => {
...
>
	<header class="col-span-4 text-center text-2xl font-bold text-green-800">Nuovo Prodotto</header>
	<!-- Titolo -->
