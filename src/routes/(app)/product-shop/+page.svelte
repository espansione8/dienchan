					...
					<div class="pt-3 border-t border-base-200 mb-3">
						<form method="POST" action="?/downloadCatalog" use:enhance={formSubmitCatalog}>
							...
							<button type="submit" class="btn btn-success w-full gap-2 group relative overflow-hidden text-white">
								<span
									class="absolute inset-0 bg-success/20 transform origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-300 ease-out"
								></span>
								<FileDown size={16} class="relative z-10" />
								<span class="relative z-10">Scarica Catalogo PDF</span>
							</button>
						</form>
					</div>
					<!-- Reset Button -->
