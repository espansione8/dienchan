...
export const actions: Actions = {
	...
	downloadCatalog: async ({ request, fetch }) => {
		...
		const query: any = { status: 'enabled', type: 'product', prodId: { $nin: idRemove } };
		if (category) query.category = category;
		if (searchTerm) query.title = { $regex: searchTerm, $options: 'i' };
		...
		try {
			...
			// Optimize all images in parallel BEFORE creating PDF
			const optimizeImage = async (imagePath: string): Promise<Buffer> => {
				try {
					const image = sharp(imagePath);
					const metadata = await image.metadata();

					// If has transparency, flatten to white background first
					...
					// No transparency, convert directly to JPEG
					return await image
						.resize(200, 200, {
							fit: 'inside',
							withoutEnlargement: true
					...
					const fallbackPath = path.join(process.cwd(), 'uploads/no_img.jpg');
					//return fs.readFileSync(fallbackPath);
					return await readFile(fallbackPath);
				}
			};
			const imagePromises = products.map(async (product: any) => {
				const productId = product.prodId.toString();
				const primaryFile = product.uploadfiles?.find((f: any) => f.type === 'product-primary')?.fileName;
				const imgPath = primaryFile
					? path.join(process.cwd(), 'uploads/product', productId, primaryFile)
				...
				return optimizeImage(imgPath);
			});

			const optimizedImages = await Promise.all(imagePromises);

			// Load logo once
			const logoBuffer = await optimizeImage(path.join(process.cwd(), 'uploads/dienchan_logo.jpg'));
			...
			products.forEach((product: any, i: number) => {
				productContent.push({
					columns: [
						{
							image: optimizedImages[i], // Use pre-optimized buffer
							fit: [80, 80],
							width: 85
						},
						{
							stack: [
								...
								{
									text: `Categoria: ${product.category?.[0] || 'N/A'}`,
									fontSize: 8.5,
									color: '#333333',
									margin: [10, 0, 0, 3]
								},
								{
									text: `Codice Prodotto: ${product.prodId || 'N/A'} - SKU: ${product.sku || 'N/A'}`,
									fontSize: 7.5,
									color: '#555555',
									margin: [10, 0, 0, 2]
								},
								...
								{
									text: `€ ${(product.price || 0).toFixed(2)}`,
									fontSize: 14,
									bold: true,
									color: '#008000',
									margin: [10, 0, 0, 5]
