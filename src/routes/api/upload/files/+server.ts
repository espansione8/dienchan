// ${import.meta.env.VITE_BASE_URL}/api/upload/files
...
const FILES_DIR = 'uploads' //`${import.meta.env.VITE_UPLOAD_URL}/${crypto.randomUUID()}`
...
export const DELETE = async ({ request }) => {
    ...
    fs.unlink(path, (err) => {
        if (err) {
            ...
            return json(
                {
                    message: 'User file delete error'
                },
                {
                    status: 500
                }
