	...
	try {
		const mailer = async () => {
			...
			const info = await transporter.sendMail({
				from: '"Fast Track IP" <info@fast-track-ip.com>', // sender address
				to: body.email, // list of receivers
				subject: 'temporary access', // Subject line
				text: `Dear user ${body.email}, we received your password reset request. Please use: ${body.password} to login and change the password with a new one in your profile`, // plain text body
				html: `
				<!DOCTYPE html>
				<html
					lang="en"
											...
											<table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%">
												<tr>
													<td style="padding: 0 2.5em; text-align: center; padding-bottom: 3em">
														<div class="text">
															<h2>Dear user ${body.email},</h2>
															<h4>we received your password reset request</h4>
															<h4>please use:<br>${body.password}</h4>
															<h4>to login and set your new password with from <b>PROFILE</b> section</h4>
															<p>Best regards</p>
															<p>Fast Track IP</p>
														</div>
													</td>
		...
		return json$1({
			message: 'temporary password have been sent'
		});
	} catch (err) {
		console.log('recoverUser ERROR:', err);
		...
		return new Response(JSON.stringify({ errors: `recoverUser ERR: ${err}` }), {
			status: 500,
			headers: {
