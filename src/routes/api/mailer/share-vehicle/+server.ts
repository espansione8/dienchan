	...
	try {
		const mailer = async () => {
			...
			const info = await transporter.sendMail({
				...
				subject: `${body.brand} ${body.model} ${body.constructionYear} Digital ID`, // Subject line
				attachments: [
					{
						filename: 'qr.png',
						path: body.QRcode,
				...
				text: `Shared Vehile digital ID ${body.pageUrl}`, // plain text body
				html: `
				<!DOCTYPE html>
				<html
					lang="en"
											...
											<table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%">
												<tr>
													<td style="padding: 0 2.5em; text-align: center; padding-bottom: 3em">
														<div class="text">
															<h2>Hello,</h2>
															<h2>someone shared with you a vehicle digital ID</h2>
														</div>
													</td>
												</tr>
												<tr>
													<td style="text-align: center">
														<div class="text-author">
															<h3 class="name">${body.brand} ${body.model} ${body.constructionYear}</h3>
															<span class="position">ID ${body.documentPageId}</span>
															<h4>CHECK THE PROFILE ONLINE</h4>
															<p>
																<a href="${body.pageUrl}" class="btn btn-primary"
																	>View ID</a
																>
															</p>
															<h4>OR <br />use the QR code below</h4>
