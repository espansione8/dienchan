...
import { Courses } from '$lib/models/Courses.model.js';
import dbConnect from '$lib/database';
//import type { RequestHandler } from '@sveltejs/kit';

///BASE_URL/api/user/findUserId/:value

export const GET = async ({ params }) => {
	try {
		await dbConnect();
		const findCourse = await Courses.findOne({ prodCorsoId: params.value }).lean().populate('userView').exec();
